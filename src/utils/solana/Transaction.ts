...
import { CompiledInstruction, Keypair, Message, PublicKey, Transaction, TransactionInstruction } from "@solana/web3.js";

export const DefaultSignature = Array(64).fill(0);
...
export async function signTransaction(tx: Transaction, key: Keypair) : Promise<Uint8Array> {
    const message = compileMessage(tx);
    const buf = message.serialize();
    return await ed25519.sign(buf, key.secretKey.subarray(0, 32));
}

export function getTransactionInspectLink(tx: Transaction): string {
    const signatures = tx.signatures.map(s => {
        let sig: any = DefaultSignature;
        if (s.signature) { sig = s.signature; }
