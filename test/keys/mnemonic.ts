...
test("mnemonic seeds (Trezor)", () => {
    ...
    for (const item of data) {
        const entropy = code.Utils.Mnemonic.toEntropy(item.phrase);
        const seed = code.Utils.Derive.seedFromMnemonic(item.phrase, password);

        assert.equal(entropy.toString('hex'), item.entropy, "Entropy mismatch");
        assert.equal(seed.toString('hex'), item.seed, "Seed mismatch");
    }
});

test("mnemonic paths (Sollet)", () => {
    const paths = [
        "m/44'/501'/0'",
        "m/44'/501'/1'",
        "m/44'/501'/2'",
    ...
    for (const path of paths) {
        const descriptor = code.Utils.Derive.descriptorFromMnemonic(new code.Utils.Path(path), phrase);
        const keypair = descriptor.toKeypair();

        assert.equal(keypair.publicKey.toBase58(), results.shift(), "Public key mismatch");
    }
});

test("mnemonic and entropy", () => {
    const data = [
        {
            phrase: "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about",
    ...
    for (const { phrase, entropy } of data) {
        assert.equal(code.Utils.Mnemonic.toEntropy(phrase).toString("hex"), entropy.toString("hex"), "Entropy mismatch");
        assert.equal(code.Utils.Mnemonic.toMnemonic(entropy), phrase, "Mnemonic mismatch");
    }
});