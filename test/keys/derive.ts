...
test("master key", () => {
    ...
    const descriptor = code.Utils.Derive.masterKey(seed);

    assert.equal(Buffer.from(descriptor.key).toString('hex'), "2b4be7f19ee27bbf30c667b642d5f4aa69fd169872f8fc3059c08ebae2eb19e7");
    assert.equal(Buffer.from(descriptor.chain).toString('hex'), "90046a93de5380a72b5e45010748567d5ea02bbf6522f979e05c0d8d8ca9fffb");
});

test("derived key (vector 1)", () => {
    // Reference: https://github.com/satoshilabs/slips/blob/master/slip-0010.md#test-vector-1-for-ed25519
    ...
    for (const item of data) {
        const path = new code.Utils.Path(item.path);
        const child = code.Utils.Derive.path(seed, path);
        const keypair = child.toKeypair();

        assert.equal(path.toString(), item.path);
        assert.equal(Buffer.from(child.chain).toString('hex'), item.chainCode);
        assert.equal(Buffer.from(child.key).toString('hex'), item.key);
        assert.equal(keypair.publicKey.toBuffer().toString('hex'), item.publicKey);
    }
});

test("derived key (vector 2)", () => {
