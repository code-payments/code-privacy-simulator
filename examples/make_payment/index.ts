...
export async function example_send_payment(
    organizer: code.Organizer,
    destination: PublicKey,
    amount: bigint,
): Promise<void> {

    // Send kin from the organizer slots to the temporary outgoing account (the
    // assumption is that these accounts have enough kin in them)
    ...
    // Create the intent
    const intent = await organizer.getSendIntent(payment);
